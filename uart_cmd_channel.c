...
extern void write_protect(int lock);
...
void process_uart_cmd_channel(UINT8 bFlag)
{
	...
	while (xyz[0] == '\r' || xyz[0] == '\n')
 	...
 	else if (strncmp(xyz, "write", 5) == 0)
 	{
		...
		// Set Buffer
		memset((byte *)(FLASH_MEM_BASE), bPATTERN, 4096); //write 4096 byte to memory
		pSRamBuffer[0] = 0;
 	...
 	else if (strncmp(xyz, "read", 4) == 0)
 	{
        ...
        word wBlkNo;
     	dword *pSRamBuffer;
		int num, poi; //num is column, poi is pSRamBuffer's pointer
 	    pSRamBuffer = (dword *)(FLASH_MEM_BASE);
		...
		for(num = 0, poi = 0; num < 256; ++num, poi+=4) //256*4*4 byte is one page
		{
			//print all data for the page
			UARTprintf(" UART pBuf[%d] 0x %08X %08X %08X %08X\n", poi, pSRamBuffer[poi], pSRamBuffer[poi+1], pSRamBuffer[poi+2], pSRamBuffer[poi+3]);
		}
		UARTputs("end>\r\r");

 	}
	else if (strncmp(xyz, "info", 4) == 0) //setting "info" instruction
 	{	
		//print Nmae and StudentID
		UARTprintf("Name : Chi-Yun,Yu\n");
		UARTprintf("StudentID : N96114221\n");
		UARTputs("end>\r\r");
 	}
	else if (strncmp(xyz, "protectEnable", 13) == 0) //setting "protectEnable" instruction
 	{	
		write_protect(0);
		//write_flag = 0;
		UARTprintf("Write is Lock\n");
		UARTputs("end>\r\r");
 	}
	else if (strncmp(xyz, "protectDisable", 14) == 0) //setting "protectDisable" instruction
 	{	
		write_protect(1);
		//write_flag = 1;
		UARTprintf("Write is unLock\n");
		UARTputs("end>\r\r");
 	}
 	else
	{
